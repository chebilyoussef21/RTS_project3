...
/* Arduino_FreeRTOS on the Mega exposes a smaller public header set than
 * desktop/full FreeRTOS ports, so this sketch only includes the headers that
 * are actually available and used here. */
#include <Arduino.h>
...
#include <semphr.h>
#include <queue.h>
#include "FreeRTOSConfig.h" /* Use the sketch-local config so the scheduler and port agree. */
...
#define schedSCHEDULING_POLICY_DMS 0 		/* Deadline-monotonic scheduling */
